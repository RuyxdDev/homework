...
let testNum = Number(prompt("Введите число:", ""));
const ISEVEN = (testNum % 2) == 0 ? alert('чет') : alert('нечет'); 
...
const incorrectWeight = (weight <= 0) ? alert("Некорректный вес посылки") : alert("Вес посылки корректный");

let deliverType = prompt("Введите тип доставки:","");

const incorrectDeliverType = (deliverType == "Стандарт" || deliverType == "Экспресс" || deliverType == "Премиум") ? alert(`Тип доставки ${deliverType}`) : alert("Неверный тип доставки");

const baseCost = weight < 1 ?  5 : (weight >= 1 && weight <= 5) ? 10 : weight > 5 ? 15 : '';
let ratio = 0;
switch (deliverType) {
    case "Стандарт":
        ratio = 1;
...
const FINALCOST = baseCost*ratio;
